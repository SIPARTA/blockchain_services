...
from blockchain_services.config import BlockchainConfig, load_abi
...
class PolygonAmoyClient:
    ...
    def _safe_total(self) -> Optional[int]:
        try:
            return int(self.get_total_data())
        except Exception:  # noqa: BLE001 - informational only
            return None
    ...
    def send_sensor_data(
        ...
        """Store one detection on-chain; returns tx hash/block/record id.

        Never raises for application logic problems without wrapping them
        into a :class:`BlockchainError` subclass first.
        """
        if self._w3 is None:
            self.connect()
        self._require_connected()
        ...
        if classification not in VALID_CLASSIFICATIONS:
            raise BlockchainError(
                ...
                f"{sorted(VALID_CLASSIFICATIONS)}, got {record.get('classification')!r}"
            )

        payload = (
            _to_uint(record.get("timestamp"), "timestamp"),
            _to_uint(record.get("mics5524"), "mics5524"),
            _to_uint(record.get("tgs2600"), "tgs2600"),
            _to_uint(record.get("mq2"), "mq2"),
            _to_uint(record.get("mq135"), "mq135"),
            classification,
            str(record.get("image_url", "")),
        )
        fn = self._w3.eth.contract(  # re-bind so linters see types
            address=self._contract.address, abi=self._contract.abi,
        ).functions.addData(*payload)

        logger.info("%s Sending sensor data...", TAG)
        ...
        if status != 1:
            raise TransactionFailedError(
                f"transaction reverted on-chain: {tx_hash}"
            )
        ...
        logger.info("%s Status: SUCCESS%s",
                    TAG,
                    f" (record #{record_id})" if record_id is not None else "")
    ...
    def get_total_data(self) -> int:
        """Total records stored in the contract."""
        self._require_connected()
        return int(self._contract.functions.getTotalData().call())

    def get_data(self, record_id: int) -> dict[str, Any]:
        """Fetch one record by id as a plain dict."""
        self._require_connected()
        if record_id < 0:
            raise BlockchainError("record id must be >= 0")
        try:
            raw = self._contract.functions.getData(int(record_id)).call()
        except ValueError as exc:
            raise BlockchainError(
                f"getData({record_id}) reverted: {self._decode_revert(exc)}"
            ) from exc
        return {
            "id": int(record_id),
            "timestamp": int(raw[0]),
